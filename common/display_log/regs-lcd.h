...
#define s5p_BASE_LCD			0xF8000000
#define  s5p_LCDREG(x)			(*(volatile unsigned long *)(s5p_BASE_LCD + (x)))
...
#define s5p_PRTCON		        s5p_LCDREG(0x0C)	/* Protection control register */
...
#define  s5p_WINSHMAP		s5p_LCDREG(0x34)        //Specifies window shadow control register.
...
#define s5p_WINCONx_BURSTLEN_MASK			(3<<9)
...
/*HOST IF registers */
/* Host I/F A - */
#define s5p_HOSTIFAREG(x)				((x) + s5p64XX_VA_HOSTIFA)
#define s5p_HOSTIFAREG_PHYS(x)				((x) + s5p64XX_PA_HOSTIFA)

/* Host I/F B - Modem I/F */
#define s5p_HOSTIFBREG(x)				((x) + s5p64XX_VA_HOSTIFB)
#define s5p_HOSTIFBREG_PHYS(x)				((x) + s5p64XX_PA_HOSTIFB)
