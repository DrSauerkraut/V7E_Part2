...
#define CONFIG_BOOTARGS    	"console=ttySAC2,115200 root=/dev/mmcblk0p2 rootfstype=ext3 init=/init" 
...
#define CFG_PROMPT              "TNS-X # "   /* Monitor Command Prompt       */
...
#define DMC0_MEMCONFIG_1	0x30F01313	// MemConfig1
...
#define CONFIG_IDENT_STRING	" for TNS-X"
...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	\
 	"_prg_bl=fuse uboot ${uboot_addr} ${filesize}; run clear\0" \
 	"_prg_kn=fuse kernel ${kernel_addr} ${filesize}; run clear\0" \
 	"_prg_ramdisk=fuse ramdisk ${ramdisk_addr} ${filesize}; run clear\0" \
 	"_prg_recovery=fuse recovery ${recovery_addr} ${filesize};run clear\0" \
 	"_prg_logo=fuse logo ${logo_addr} ${filesize}; run clear\0" \
	\
	...
	\
	"load_logo_sd=movi 1 init; fatload mmc 1:1 ${logo_addr} logo.img\0" \
	...
	\
	"prg_logo_sd=movi 1 init; fatload mmc 1:1 ${logo_addr} logo.img; run _prg_logo\0" \
 	"prg_logo_dnw=dnw ${logo_addr}; run _prg_logo\0" \
	\
	"format_inand=fatformat mmc 0:1; ext3format mmc 0:2; ext3format mmc 0:3; ext3format mmc 0:4\0" \
	"bootargs_android=setenv bootargs console=ttySAC2,115200 root=/dev/ram0 rw ramdisk=4608 init=/init\0" \
	"bootcmd_smt=movi 1 init; movi 0 init; run prg_uboot_sd; run prg_kernel_sd; run prg_recovery_sd; reset\0" \
