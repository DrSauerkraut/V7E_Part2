...
//#define CONFIG_BSP_VERSION_STRING	"BSP version 0.1.000"
...
#if defined(CONFIG_PRODUCT_PDC3)
#define CFG_PROMPT              "PD-C3 # "   /* Monitor Command Prompt       */
#define CFG_BOARD_NAME		"PD-C3"

#else
#define CFG_PROMPT              "PD-C2 # "   /* Monitor Command Prompt       */
#define CFG_BOARD_NAME		"PD-C2"
#endif
...
#define DMC0_MEMCONFIG_1	0x00F01313	// MemConfig1
...
#define DMC1_MEMCONFIG_1	0x00F01213	// MemConfig1
 ...
 */
#if defined(CONFIG_PRODUCT_PDC3)
#define CONFIG_IDENT_STRING	" for PD-C3"
#else
#define CONFIG_IDENT_STRING	" for PD-C2"
#endif
...
#define CONFIG_MMC_CHANNEL0
//#define CONFIG_MMC_CHANNEL1
//#define CONFIG_MMC_CHANNEL2
#define CONFIG_SHARE_REGION
#define CONFIG_MMC_PARTITION

#ifdef CONFIG_MMC_CHANNEL0
#define CONFIG_CMD_SMT
#endif

#define setenv_filesize(size)	\
{	\
	char buf[12];	\
	sprintf(buf, "%lX", size);	\
	setenv("filesize", buf);	\
...
//#define CONFIG_LOW_BATT_LOGO
#define CONFIG_EXTRA_ENV_SETTINGS \
 	"filesize=0\0" \
  	"uboot_addr=30000000\0" \
  	"kernel_addr=30008000\0"      \
  	"kernel_start=30008040\0"      \
  	"ramdisk_addr=31000000\0" \
  	"recovery_addr=31000000\0" \
 	"logo_addr=32000000\0" \
	"temp_addr=32200000\0" \
	"charlib_addr=32200300\0" \
  	"clear=setenv filesize 0\0"  \
	\
 	"_prg_bl=fuse uboot1 ${uboot_addr} ${filesize};fuse uboot2 ${uboot_addr} ${filesize}; run clear\0" \
 	"_prg_kn=fuse kernel1 ${kernel_addr} ${filesize};fuse kernel2 ${kernel_addr} ${filesize}; run clear\0" \
 	"_prg_ramdisk=fuse ramdisk1 ${ramdisk_addr} ${filesize};fuse ramdisk2 ${ramdisk_addr} ${filesize}; run clear\0" \
 	"_prg_recovery=fuse recovery1 ${recovery_addr} ${filesize};fuse recovery2 ${recovery_addr} ${filesize};run clear\0" \
 	"_prg_logo1=fuse logo1 ${logo_addr} ${filesize}; run clear\0" \
	"_prg_logo2=fuse logo2 ${logo_addr} ${filesize}; run clear\0" \
	"_prg_logo3=fuse logo3 ${logo_addr} ${filesize}; run clear\0" \
	\
 	"load_uboot_sd=movi 1 init; fatload mmc 1:1 ${uboot_addr} u-boot.bin\0" \
	"load_bsp_sd=movi 1 init; fatload mmc 1:1 ${temp_addr} bspfuse\0" \
	"load_char_sd=fatload mmc 1:1 ${charlib_addr} char.bin\0" \
 	"load_uboot_dnw=dnw ${uboot_addr}\0" \
 	"load_uboot=load uboot ${uboot_addr}\0" \
	\
	"load_kernel_sd=movi 1 init; fatload mmc 1:1 ${kernel_addr} uImage\0" \
 	"load_kernel_dnw=dnw ${kernel_addr}\0" \
 	"load_kernel=load kernel ${kernel_addr}\0" \
	\
	"load_ramdisk_sd=movi 1 init; fatload mmc 1:1 ${ramdisk_addr} ramdisk-uboot.img\0" \
 	"load_ramdisk_dnw=dnw ${ramdisk_addr}\0" \
 	"load_ramdisk=load ramdisk ${ramdisk_addr}\0" \
	\
	"load_recovery_sd=movi 1 init; fatload mmc 1:1 ${recovery_addr} ramdisk-recovery-uboot.img\0" \
 	"load_recovery_dnw=dnw ${recovery_addr}\0" \
 	"load_recovery=load recovery ${recovery_addr}\0" \
	\
	"load_logo_sd=movi 1 init; fatload mmc 1:1 ${logo1_addr} logo1.img\0" \
 	"load_logo_dnw=dnw ${logo_addr}\0" \
	\
 	"prg_uboot_sd=run load_uboot_sd; run _prg_bl\0" \
 	"prg_uboot_dnw=run load_uboot_dnw; run _prg_bl\0" \
	\
	"prg_kernel_sd=run load_kernel_sd; run _prg_kn\0" \
 	"prg_kernel_dnw=run load_kernel_dnw; run _prg_kn\0" \
	\
	"prg_ramdisk_sd=run load_ramdisk_sd; run _prg_ramdisk\0" \
 	"prg_ramdisk_dnw=run load_ramdisk_dnw; run _prg_ramdisk\0" \
	\
	"prg_recovery_sd=movi 1 init; fatload mmc 1:1 ${recovery_addr} ramdisk-recovery-uboot.img; run _prg_recovery\0" \
 	"prg_recovery_dnw=dnw ${recovery_addr}; run _prg_recovery\0" \
	\
	"prg_logo1_sd=movi 1 init; fatload mmc 1:1 ${logo_addr} logo1.img; run _prg_logo1\0" \
	"prg_logo2_sd=movi 1 init; fatload mmc 1:1 ${logo_addr} logo2.img; run _prg_logo2\0" \
	"prg_logo3_sd=movi 1 init; fatload mmc 1:1 ${logo_addr} logo3.img; run _prg_logo3\0" \
	"prg_logo_sd=run prg_logo1_sd; fatload mmc 1:1 ${logo_addr} logo2.img; run _prg_logo2;fatload mmc 1:1 ${logo_addr} logo3.img; run _prg_logo3\0" \
 	"prg_logo1_dnw=dnw ${logo_addr}; run _prg_logo1\0" \
	\
	"format_inand=fatformat mmc 0:1; ext3format mmc 0:2; ext3format mmc 0:3; ext3format mmc 0:4\0" \
	"bootargs_android=setenv bootargs console=ttySAC2,115200 root=/dev/ram0 rw ramdisk=4608 init=/init androidboot.serialno=8013000711700001\0" \
	"bootcmd_smt=movi 1 init; movi 0 init; run prg_uboot_sd; run prg_logo1_sd; run prg_kernel_sd; run prg_recovery_sd; reset\0" \
	"bootcmd_recovery=run bootargs_android; run load_kernel; run load_recovery; bootm ${kernel_start} ${recovery_addr}\0" \
	"bootcmd_android=run bootargs_android; run load_kernel; run load_ramdisk; bootm ${kernel_start} ${ramdisk_addr}\0" 
