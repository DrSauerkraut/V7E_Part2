...
{
#if defined(CONFIG_MMC_CHANNEL0)
	extern int get_vi_test();
	if(get_vi_test())
		return mmc_bwrite(1, start, blkcnt, src);
	else
		return mmc_bwrite(0, start, blkcnt, src);
#elif defined(CONFIG_MMC_CHANNEL1)
	return mmc_bwrite(1, start, blkcnt, src);
#elif defined(CONFIG_MMC_CHANNEL2)
	return mmc_bwrite(2, start, blkcnt, src);
#endif
}
...
ulong movi_read(ulong start, lbaint_t blkcnt, void *dst)
{
	...
	return mmc_bread(1, start, blkcnt, dst);
#elif defined(CONFIG_MMC_CHANNEL2)
	return mmc_bread(2, start, blkcnt, dst);
#endif
}
...
static int mmc_decode_csd(struct mmc *host)
{
	...
	//printf("e = %d, m = %d, csd->capacity = %d, read_bl_len = %d\n", e, m, csd->capacity, host->read_bl_len);
	return 0;
}
...
static int mmc_read_ext_csd(struct mmc *host)
{
	...
	if (ext_csd_struct >= 2) {
		...
		if (host->ext_csd.sectors) {
			...
			host->capacity = host->ext_csd.sectors/host->read_bl_len*512;
		}
	}
	...
	default:
		...
		printf("card is mmc v4 but doesn't "
			"support any high-speed modes. %d\n", ext_csd[EXT_CSD_CARD_TYPE]);
		goto out;
	}
...
int mmc_startup(struct mmc *host)
{
	...
	host->clock=host->clock/4;
	printf("%s:%d",__FUNCTION__,host->clock);
...
int mmc_send_if_cond(struct mmc *host)
{
	...
	if (err) {
		//printf("mmc_send_cmd error\n");
		return err;
	}

	if ((cmd.resp[0] & 0xff) != 0xaa) {
		printf("it is not SD_VERSION_2\n");
		return UNUSABLE_ERR;
	}
	else
...
int mmc_initialize(bd_t *bis, int dev_num)
{
	struct mmc *mmc;
	int err;
...
#if defined(CONFIG_SMB_A7002)
	mmc = find_mmc_device(0);
#elif defined(CONFIG_SMB_A1012)||defined(CONFIG_SMB_A1001)
	...
	if (mmc) {
		...
		if (err)
			err = mmc_init(mmc);
		if (err) {	
			printf("Card %d init fail!\n", dev_num);
			return 1;
		}
	}

	printf("dev %d : %ldMB\n", dev_num, (mmc->capacity/(1024*1024/mmc->read_bl_len)));
	return 0;
}
